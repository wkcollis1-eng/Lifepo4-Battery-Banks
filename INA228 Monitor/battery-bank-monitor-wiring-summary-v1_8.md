...
**Document revision:** 1.8 (**topology reverted high-side → low-side**; **board changed V1 carrier → V2 native-INA228**; tracks firmware V1.15; §13F BOM placeholders resolved; §13D pin-map defect closed)
...
Firmware: `battery-bank-monitor.yaml` (**V1.15**). The V2 pin-map update (§6.1, §13D) is **implemented in V1.15** — no further firmware edit required before flashing to a V2 board. The measurement logic, sign convention, parameters, robustness guards, and display behavior are otherwise unchanged from Rev 1.6.

### 6.1 Pin Assignments — V2 update (implemented V1.15)

| Function | V1.14 firmware (V1 carrier) | **V2 board / V1.15 firmware** | Status |
|---|---|---|---|
...
| INA228 ALERT | unused (GPIO20 dead-trace clamp) | **GPIO5 (D3)** — provisioned, not wired | optional; now unblocked |
| GPIO20 clamp | clamps floating ex-ALERT trace | removed — GPIO20 is true NC on V2 | **removed in V1.15** |
...
- [ ] **Firmware V1.15 compiled** (button → GPIO4 and GPIO20 clamp removal already incorporated)
...
| Document | Source |
|---|---|
| Battery-bank firmware V1.15 | `battery-bank-monitor.yaml` |
...
## 13. Open Risks / Pre-Fab To-Do (updated Rev 1.8)

*Items D and F closed in Rev 1.8 / firmware V1.15. Remaining open items: C (accepted), E (discipline), G (deferred), H (verify at build), I (cosmetic), J (confirm at build).*
...
| **D** | ~~Firmware pin-map mismatch. Button on GPIO4 (board) vs GPIO21 (firmware); ALERT now on GPIO5; GPIO20 clamp vestigial.~~ — **CLOSED in V1.15.** Button changed to GPIO4; GPIO20 clamp removed. | ~~High~~ **Closed** | Done. Flash V1.15. |
