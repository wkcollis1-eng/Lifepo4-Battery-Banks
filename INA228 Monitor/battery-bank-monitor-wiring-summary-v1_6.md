...
**Document revision:** 1.6 (high-side topology; INA228-in-INA260-footprint adaptation; Kelvin-sense fusing; local panel display; tracks firmware V1.14)
...
| 1.6 | **Tracks firmware V1.14** (local panel display + wake button). No topology change; adds one display, one cable, one button. Implements the **local SOC-readout** that was a v1.5 commissioning to-do — closing the gap where SOC was visible only via Wi-Fi → HA → phone. New **§6.6 Local panel display** documents the hardware (Adafruit 938 SSD1306 128×64 I²C on the existing bus, Adafruit 4209 STEMMA-QT→Dupont cable, Adafruit 1505 wake button), the wiring, the four button-cycled pages, the 5-min auto-sleep, and the burn-in rationale. §6.1 pin table updated (GPIO21 now the wake-button input). §8.1 checklist gains display bring-up and bus-pull-up verification. Records the **dry-basement** siting decision that settles the non-sealed button choice. Firmware references updated V1.13 → V1.14. |
...
In ESPHome firmware (V1.14 uses the `ina2xx_i2c` platform):
...
The existing battery-bank monitor firmware (`battery-bank-monitor.yaml`, **V1.14**) is configured for the high-side topology. *This section is substantially rewritten from V1.2 §6 — V1.2 was forward-looking from the UPS-baseline migration perspective; V1.3 references the production firmware that resulted from that work.*

### 6.1 Pin Assignments (XIAO ESP32-C3, esp-idf framework)

| Pin | Function | Notes |
|---|---|---|
| GPIO6 | I²C SDA | To INA228 SDA (pin 4 of breakout header) — **shared bus**; also the SSD1306 display SDA (V1.14) |
| GPIO7 | I²C SCL | To INA228 SCL (pin 3 of breakout header) — **shared bus**; also the SSD1306 display SCL (V1.14) |
| GPIO10 | DS18B20 1-Wire | To TB2.2 |
| GPIO20 | Unused (was INA260 ALERT in original board design) | **(V1.3) U2 pin 5 lifted, R2 DNP, trace floating. Firmware enables ESP32-C3 internal pullup to clamp the pin via `binary_sensor: gpio` with `internal: true`.** Not usable for the button — it is tied to a dead PCB trace. |
| GPIO21 | **Display wake button (V1.14)** | XIAO TX pad, freed by `logger: baud_rate: 0`. Non-strapping, no other role in production. To Adafruit 1505 momentary button → GND; `INPUT_PULLUP`, pressed = LOW. |
...
### 6.3 System Parameters (V1.14 firmware values)

The V1.14 firmware substitutions are tuned to the actual measured bank (V1.2 had these as forward-looking placeholders):
...
### 6.6 Local Panel Display (V1.14)

*This implements the **local SOC-readout** flagged as a v1.5 commissioning to-do. In the manual model the shutdown decision depends on reading SOC, and previously SOC was visible only through the Wi-Fi → HA → phone path. During an outage that path is powered (bank → inverter → UPS → HA Green + modem) so it usually works, but it has several failure points and no fallback at the panel. A small local display, mounted in the enclosure and read at the panel where the operator throws the breaker, removes that single dependency — the readout survives any network loss.*

**Bill of materials (3 added parts):**

| Item | Part | Notes |
|---|---|---|
| Display | Adafruit 938 — Monochrome 1.3″ 128×64 OLED, STEMMA QT | SSD1306 controller, I²C default, address 0x3C, level-shifted, onboard regulator, ~40 mA when lit |
| Cable | Adafruit 4209 — STEMMA QT → 4-pin male Dupont, 150 mm | Solderless on the display end |
| Button | Adafruit 1505 — 16 mm panel-mount momentary, black | Normally-open micro-switch; needs a 16 mm hole, ~30 mm depth behind panel |

**Wiring — two independent circuits.** The display and the button are *separate*; the button is **not** in the display's power path (the panel is powered continuously and slept in firmware, not switched by the button).

*Display (4209 cable, QT → board I²C):*
- Red → 3V3 (XIAO 3V3 pad, or the 3.3 V node the INA228 already uses — same rail)
- Black → GND
- Blue → **GPIO6** (SDA — the existing `bus_a`, shared with the INA228)
- Yellow → **GPIO7** (SCL — the existing `bus_a`)

*Button (2 wires):*
- One terminal → **GPIO21**
- Other terminal → **GND**

No 3.3 V goes to the button; the ESP32-C3 internal pull-up holds GPIO21 high and the press shorts it to GND. The display shares SDA/SCL with the INA228 — normal I²C bus-sharing, two devices at different addresses (display 0x3C, INA228 0x40).

> **Bus pull-up caution.** The 938 has onboard I²C pull-ups, and so does the INA228 breakout. Two sets in parallel lower the effective pull-up resistance. At 100 kHz on short runs this is normally fine, but if the bus misbehaves with both connected (garbled glyphs, or the INA228 NACKing), the first fix is to remove the 938's pull-up resistors (small SMD parts on the back). Verify the bus with **both** devices connected during commissioning (§8.1).

**Mounting (cover-mount, no cutout).** The LeMotech enclosure has a **clear polycarbonate cover**, and the OLED reads cleanly through clear PC — so do **not** cut a window (a cutout would destroy the IP65 seal the box provides). Fix the 938 to the inside face of the clear cover with double-sided foam tape or adhesive standoffs, display facing out, pressed close to the PC to minimize parallax. The flying lead reaches from the board on the floor to the cover with a service loop so the lid still opens. The **button goes through the ABS sidewall**, not the cover. The interior floor (~108 × 83 mm; box is 115 × 90 × 55 mm) has ample room beside the 78 × 46 mm board, and 55 mm depth clears the button body.

> **Siting / seal note (V1.6).** This unit is installed in a **dry, humidity-controlled basement**. The Adafruit 1505 button is **not** gasketed (no IP rating), so drilling it into the sidewall leaves a non-sealed hole — acceptable here because the location is dry, dust-only. *If this enclosure is ever relocated to a damp or outdoor environment, swap 1505 for a gasketed 16 mm metal momentary button (same 16 mm hole, same 2-wire wiring) to preserve IP65.*

**Behavior.** OFF by default (the panel starts dark at boot). First button press wakes to page 1; each subsequent press cycles to the next page (wraps at the end). The panel auto-sleeps after the `display_timeout` substitution (**default 5 min**), measured from the **last** press (a restart-mode script resets the countdown on every press).

> **Burn-in tradeoff (operator note).** SSD1306 panels dim with prolonged on-time, which is why the display is dark except on demand. A 5-minute timeout per press is convenient when watching the trend toward 20%, but if checked many times during an outage it accumulates on-time. `display_timeout` is a substitution: shorten it to `1min` if panel longevity is preferred over dwell time — no other change needed. (Firmware uses the display `turn_on()`/`turn_off()` API for sleep/wake, never `setup()`, which avoids a known ESPHome crash from repeated re-init calls.)

**Pages (4, button-cycled):**

1. **V / I / W / SOC** — SOC in large type (the decision variable). Current is **signed and direction-labeled** (`CHG` / `DISCHG` / `IDLE`) so "still draining, and how fast" is explicit rather than a bare magnitude.
2. **Runtime remaining + voltage slope + knee/cliff** — estimated time at the current load, the EMA slope (mV/min), and the knee/cliff flags. Turns a static SOC into "time to act." Shows "not discharging" when runtime is undefined (charging/idle).
3. **Health / trust** — `bank_state`, INA228 watchdog, **Current Channel Fault** (V1.13), DS18B20 health, and battery temperature. This is the "is the SOC trustworthy?" page — most relevant exactly when the HA path is down and the operator is relying solely on the panel. If the current channel has faulted, page 1's SOC is stale, and this page is how the operator knows not to trust it.
4. **Charge / absorption** — charge current and the `absorption_reached` flag, with an explicit "OK to switch charger off → SOC anchors 100%" cue once absorption is confirmed. This backstops the re-anchoring procedure (charge to **confirmed** absorption, *then* switch the charger off) that is otherwise invisible at the panel — see §10.2.

**Isolation from CORE.** The display only **reads** published states; it never writes the alarm path, the `bank_state` machine, or the SOC counter. A display or button fault cannot affect backup readiness or SOC integrity.
...
- [ ] Battery-bank ESPHome YAML (V1.14) compiled with correct topology configuration
- [ ] **(V1.6 addition)** Display bring-up: 938 enumerates at 0x3C on `bus_a` (alongside INA228 at 0x40); button press wakes the panel and cycles all four pages; panel auto-sleeps after 5 min
- [ ] **(V1.6 addition)** I²C bus integrity verified with **both** INA228 and display connected (no NACKs, no garbled glyphs) — if flaky, remove the 938's onboard pull-ups first (see §6.6)
...
The V1.14 firmware maintains Coulomb counting anchored at each confirmed full charge (V1.7+ feature). Recalibration is automatic at full-charge anchors. The detection is two-stage:
