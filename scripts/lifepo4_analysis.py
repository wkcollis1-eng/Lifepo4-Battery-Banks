...
if len(stasis_df) > 1:
    ...
    slope_full, intercept_full, r_full, p_full, se_full = stats.linregress(stasis_df['days'], stasis_df['Mid'])
    total_days_full = stasis_df['days'].max()
    total_change_full = slope_full * total_days_full
    residuals_full = stasis_df['Mid'] - (intercept_full + slope_full * stasis_df['days'])
...
if len(last30_df) > 1:
    ...
    slope_30, intercept_30, r_30, p_30, se_30 = stats.linregress(last30_df['days'], last30_df['Mid'])
...
minute_stats = hf_analysis.groupby('minute').agg({
    'voltage': ['mean', 'std', 'min', 'max', 'count'],
    'MA60': ['mean', 'std']
}).reset_index()
minute_stats.columns = ['minute', 'v_mean', 'v_std', 'v_min', 'v_max', 'samples', 'ma60_mean', 'ma60_std']
...
for name, start, end in segments:
    ...
    if len(seg) > 0:
        ...
        print(f"  {name}: {len(seg):,} samples | Raw σ: {raw_s:.2f} mV | MA-60s σ: {ma60_s:.2f} mV | Reduction: {reduction:.1f}%")
...
merged = pd.merge(hourly_df[['datetime', 'Mid']], temp_df[['datetime', 'TempMid']], 
                  on='datetime', how='inner')
print(f"\nMatched hourly dataset: {len(merged)} points")
print(f"  Temperature range: {merged['TempMid'].min():.2f}°F → {merged['TempMid'].max():.2f}°F (Δ{merged['TempMid'].max() - merged['TempMid'].min():.2f}°F)")
...
if len(merged) > 2:
    slope_t, intercept_t, r_t, p_t, se_t = stats.linregress(merged['TempMid'], merged['Mid'] * 1000)
    print(f"\nNaive Regression (V vs T only):")
    print(f"  Temperature coefficient: +{slope_t:.2f} mV/°F")
    print(f"  R²: {r_t**2:.3f}")
    
    # Two-factor regression (time + temperature)
    merged['days_from_start'] = (merged['datetime'] - merged['datetime'].min()).dt.total_seconds() / 86400
...
before_eco = hourly_df[(hourly_df['datetime'] >= eco_before_start) & (hourly_df['datetime'] < eco_before_end)]
after_eco = hourly_df[(hourly_df['datetime'] >= eco_after_start) & (hourly_df['datetime'] <= eco_after_end)]
